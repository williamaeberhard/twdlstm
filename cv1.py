...
print(now_str + ' running twdlstm cv1 v0.6.4\n')
# print('\n')
...
#%% "CV iterations over i index" adapted to single fold (suplied in config)
...
i = int(config['i_cv']) # i index identifies held-out series, in [0,1,...,nb_series-1]
...
bias_tr = np.mean(ytr) - np.mean(ytr_pred)
scale_tr = np.std(ytr)/np.std(ytr_pred)
r_tr = np.corrcoef(ytr, ytr_pred)[0,1]
r2_tr = r2_score(ytr, ytr_pred)
MedAE_tr = median_absolute_error(ytr, ytr_pred)
...
bias_va = np.mean(yva) - np.mean(yva_pred)
scale_va = np.std(yva)/np.std(yva_pred)
r_va = np.corrcoef(yva, yva_pred)[0,1]
r2_va = r2_score(yva, yva_pred)
MedAE_va = median_absolute_error(yva, yva_pred)
...
print('--- CV fold',i) # 
print('* tr metrics:') # 
print('  - mean(obs)-mean(pred) =',round(bias_tr,4)) # diff of means
print('  - sd(obs)/sd(pred) =',round(scale_tr,4)) # ratio of scales
print('  - lin corr =',round(r_tr,4)) # lin corr
print('  - R^2 =',round(r2_tr,4)) # R^2 on te batches, by series
print('  - MedAE =',round(MedAE_tr,4)) # R^2 on te batches, by series
print('* te metrics:') # 
print('  - mean(obs)-mean(pred) =',round(bias_va,4)) # diff of means
print('  - sd(obs)/sd(pred) =',round(scale_va,4)) # ratio of scales
print('  - lin corr =',round(r_va,4)) # lin corr
print('  - R^2 =',round(r2_va,4)) # R^2 on te batches, by series
print('  - MedAE =',round(MedAE_va,4)) # R^2 on te batches, by series
