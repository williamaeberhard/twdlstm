...
class Model_LSTM(torch.nn.Module):
    def __init__(self, input_size, d1_hidden, d_hidden, d2_hidden, num_layers, output_size, p_drop):
        ...
        self.d_hidden = d_hidden
        self.d2_hidden = d2_hidden
        self.num_layers = num_layers
        self.p_drop = p_drop
        self.dp = torch.nn.Dropout(p=self.p_drop)
        self.fc1 = torch.nn.Linear(
            in_features=input_size,
            out_features=self.d1_hidden
        )
        self.act1 = torch.nn.Tanh() # torch.nn.ReLU()
        self.lstm = torch.nn.LSTM(
            input_size=self.d1_hidden,
        ...
        self.fc2 = torch.nn.Linear(
            ...
            out_features=self.d2_hidden
        )
        self.act2 = torch.nn.Tanh() # torch.nn.ReLU()
        self.linear = torch.nn.Linear(
            in_features=self.d2_hidden,
            out_features=output_size
        )
